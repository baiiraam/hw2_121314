...
created after this loop, so it has `requires\_grad=True` by default.

Parameter counts (from `count\_trainable\_params()`):
...
I encountered two significant bugs during development.

First, the most critical bug was in B3's test evaluation. Originally, I used
`load\_base\_model()` to load the B2 checkpoint for all three regimes,
resulting in identical test accuracies (0.9370, 0.9370, 0.9370). The fix was
to save each regime's trained model to `augment\_{regime}.pth` and load them
with `load\_regime\_model(regime, device)` instead of the shared B2 base model.
After the fix, test accuracies differed (0.9400, 0.9170, 0.9340).

Second, I encountered a device mismatch bug when moving from Colab (GPU) to
my local machine (CPU). Models saved on GPU could not be loaded on CPU:
\begin{lstlisting}
RuntimeError: Attempting to deserialize object on a CUDA device but
torch.cuda.is_available() is False.
\end{lstlisting}
The fix was to add `map\_location=device` to all `torch.load()` calls:
\begin{lstlisting}
model.load_state_dict(torch.load("model.pth", map_location=device))
\end{lstlisting}
This made the code device-agnostic, allowing checkpoints to be loaded on
both GPU and CPU environments without modification.

\subsection{Reproducibility (C5)}

My Student ID is set to 121314 (just a number for the seed in this case). The seed flows through:
\newline
...
2. Weight initialization: PyTorch's default initialization is
   deterministic when `torch.manual\_seed(seed)` is set.
\newline
3. DataLoader shuffling: Uses PyTorch's RNG, seeded via
   `torch.manual\_seed(seed)`.
\newline
4. CUDA: `torch.cuda.manual\_seed\_all(seed)` ensures GPU determinism.

Running `python run\_all.py` twice with the same seed produces identical figures.

% =====================================================================
\section{Pretrained-Model Inference (Bonus)}

I ran pretrained torchvision models on 5 photos, including soccer players, my photos, and downloaded images. The annotated outputs are saved as
`figures/zoo\_*.png`.
