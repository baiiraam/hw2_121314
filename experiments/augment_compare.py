...
def train_regime(model, train_loader, val_loader, device, mix=None):
    """Train a regime and return history."""
...
def get_regime_history(regime_name, train_func):
    ...
    if history_exists(history_path):
        print(f"📂 {regime_name} history found. Loading...")
        return load_history(history_path)
    else:
        print(f"🔨 {regime_name} history not found. Training from scratch...")
        history = train_func()
...
def load_base_model(device):
    ...
    model.load_state_dict(torch.load("models/best_resnet_ft.pth"))
...
def main():
    ...
    def train_none():
        ...
        return train_regime(model, train_loader_no_aug, val_loader_no_aug, device)

    history_none = get_regime_history("none", train_none)
    ...
    def train_std():
        model = load_base_model(device)
        return train_regime(model, train_loader_std, val_loader_std, device)

    history_std = get_regime_history("std", train_std)
    ...
    def train_mix():
        model = load_base_model(device)
        return train_regime(model, train_loader_std, val_loader_std, device, mix="mixup")

    history_mix = get_regime_history("mix", train_mix)
    ...
    model_none = load_base_model(device)
    model_std = load_base_model(device)
    model_mix = load_base_model(device)
    ...
    print(f"No Augmentation:  Test Acc = {test_metrics_none['acc']:.4f}")
    print(f"Standard:         Test Acc = {test_metrics_std['acc']:.4f}")
    print(f"Mixup:            Test Acc = {test_metrics_mix['acc']:.4f}")
