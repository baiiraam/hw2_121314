...
def run_detection(image_path, device, score_threshold=BONUS_SCORE_THRESHOLD):
    ...
    output_path = os.path.join(
        FIGURES_DIR, f"zoo_detection_{os.path.basename(image_path).split('.')[0]}.png"
    )
...
def run_segmentation(image_path, device, score_threshold=BONUS_SCORE_THRESHOLD):
    ...
    for i, (box, label, score, mask) in enumerate(zip(boxes, labels, scores, masks)):
        ...
        # ✅ Fixed: Drop alpha channel (RGBA → RGB)
    ...
    output_path = os.path.join(
        FIGURES_DIR,
        f"zoo_segmentation_{os.path.basename(image_path).split('.')[0]}.png",
    )
    os.makedirs(FIGURES_DIR, exist_ok=True)
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
...
def run_pose(image_path, device, score_threshold=BONUS_SCORE_THRESHOLD):
    ...
    logger.info(
        f"Found {len(keypoints)} person(s) with keypoints above threshold {score_threshold}"
    )
    ...
    plt.tight_layout()
    output_path = os.path.join(
        FIGURES_DIR, f"zoo_pose_{os.path.basename(image_path).split('.')[0]}.png"
    )
    os.makedirs(FIGURES_DIR, exist_ok=True)
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    if not sample_images:
        ...
        logger.info(f"Please add 3-5 of your own photos to {SAMPLES_DIR}/")
        return
    ...
    logger.success(f"Outputs saved to {FIGURES_DIR}/zoo_*.png")
